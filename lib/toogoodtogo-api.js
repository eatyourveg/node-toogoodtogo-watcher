...
export function login() {
  ...
  console.log(session)
  return session.refreshToken ? refreshToken() : Promise.resolve();
}
...
export function listFavoriteBusinesses() {
  ...
  if (!session.refreshToken) {
    console.log(
      "You are not logged in. Login via the command `toogoodtogo-watcher login` or via the HomeAssistant MQTT button or via `/login` with the Telegram Bot",
    );
